...
def improc_tight_hist(im, lims, op_range, samples=np.empty((0, 0))):
    # Tightens the brightness level to around lims[0] and lims[1] of the
    # histogram, where 0<lims<1. op_range defines the o/p range of the image

    # Special case, when both lims are 0 are
    if np.all(lims == 0):
        lims = [im.min(), im.max()]
    else:
        # Find histogram
        if samples.size == 0:
            samples = np.arange(im.min(), im.max() + 1)
        lims = stat_hist_lims(im, lims, samples)[0]

    # Stretch the values
    df = lims[1] - lims[0]
    t_im = (im - lims[0]) * op_range / df
...
def z_norm2(im, k=31, pr=5):
    ...
    for i in range(hk, v - hk):
        for j in range(hk, s - hk):
            mean_im[i, j] = (iim[i + hk, j + hk] - iim[i - hk, j + hk] - iim[i + hk, j - hk] + iim[
                i - hk, j - hk]) / k ** 2
            var_im[i, j] = (sqiim[i + hk, j + hk] - sqiim[i - hk, j + hk] - sqiim[i + hk, j - hk] + sqiim[
                i - hk, j - hk]) / k ** 2 - mean_im[i, j] ** 2
...
def autocrop(im, disp_flag=0, oz_pov=0.2):
    # funkcija za automatsko odsecanje neozracenog regiona
    ...
    if any(np.array(im.shape) > 2380):
        work_im = np.float32(im[1:im.shape[0]:4, 1:im.shape[1]:4])
    else:
    ...
    k = np.max(np.where((np.array(work_im.shape).min() / (2 ** np.arange(1, 6))) > 50))  # k-pozicija u vektoru
    LP, GP, Res, size_vec = im_pyr_decomp(work_im, k + 1)  # k+1 - na koliko nivoa se razlaze
    work_im = Res
    uk_fak = uk_fak * (2 ** (k + 1))
    ...
    if mn_x <= k and mn_y <= k and mx_x >= s - k + 1 and mx_y >= v - k + 1:
        ...
        roi = {'x': 0, 'y': 0, 'nx': 4 * poc_s, 'ny': 4 * poc_v, 'dx': 1, 'dy': 0}
        cr_im = im
        return cr_im

    # Pronadji ivice kolimisanog dela
    # Ivice u slici

    hg, vg, sg, o = sobel_grad(work_im)
    ...
    # Odziv za vodoravne i uspravne ivice
    r_h = np.sum(np.sign(hg) * np.abs(hg), axis=0) / (np.sum(np.abs(sg), axis=0) + np.finfo(float).eps)
    r_v = np.sum(np.sign(vg) * np.abs(vg), axis=1) / (np.sum(np.abs(sg), axis=1) + np.finfo(float).eps)

    # Odbaci one koje vec znamo da nisu
    r_h[mn_x: mx_x + 1] = 0  # Odbaci granice sa ozracenim intenzitetom
    r_v[mn_y: mx_y + 1] = 0
    r_h[:k] = 0  # Odbaci granice ispod tolerancije
    r_h[s - k + 1: s + 1] = 0
    r_v[:k] = 0
    r_v[v - k + 1: v + 1] = 0

    # iz nekog razloga se obrne???

    # Nadji moguce granice (Levo, Desno, Gore, Ispod)
    g_D = np.asarray(np.where(r_h < -lin_T))[0]
    g_L = np.asarray(np.where(r_h > lin_T))[0]
    g_I = np.asarray(np.where(r_v < -lin_T))[0]
    g_G = np.asarray(np.where(r_v > lin_T))[0]

    # Odbaci one koje suvise prelaze na drugu stranu i uzmi najuzu
    # Ako ih nema uzmi kraj slike kao referentnu granicu
    if g_L.size != 0:
        mn_x = g_L[g_L < ((0.5 - cr_tol) * s)].max()
    else:
        mn_x = 0
    if g_D.size != 0:
        mx_x = g_D[g_D > ((0.5 + cr_tol) * s)].min()
    else:
        mx_x = s - 1
    if g_G.size != 0:
        mn_y = g_G[g_G < ((0.5 - cr_tol) * v)].max()
    else:
        mn_y = 0
    if g_I.size != 0:
        mx_y = g_I[g_I > ((0.5 + cr_tol) * v)].min()
    else:
        mx_y = v - 1

    mn_x = uk_fak * (mn_x + 1) + 1
    mx_x = uk_fak * (mx_x + 1)
    mn_y = uk_fak * (mn_y + 1) + 1
    mx_y = uk_fak * (mx_y + 1)

    # Ogranici na velicinu slike
    if mx_x > 4 * poc_s: mx_x = 4 * poc_s
    if mx_y > 4 * poc_v: mx_y = 4 * poc_v

    cr_im = im[mn_y - 1:mx_y, mn_x - 1:mx_x + 1]
